...
const makeNodes = (): Record<string, NodeState> => ({
...
const makeEdges = (): Record<string, EdgeState> => ({
...
export const useGraphStore = create<GraphStore>((set, get) => ({
  ...
  processEvent: (e: SynapseEvent) => {
    ...
    const alert = (msg: string, severity: AlertEvent['severity']): AlertEvent => ({
      id:        `${e.type}-${e.timestamp}-${Math.random()}`,
    ...
    switch (e.type) {
      case 'node_start':
        ...
        set({
          ...
          nodes: {
            ...s.nodes,
            [e.nodeId]: { ...s.nodes[e.nodeId], status: 'running' },
          },
          alerts: [alert(`${s.nodes[e.nodeId]?.label ?? e.nodeId} started`, 'info'), ...s.alerts],
        })
        break
      ...
      case 'handoff_start':
        ...
        set({
          edges: {
            ...s.edges,
            [e.edgeId]: { ...s.edges[e.edgeId], status: 'transmitting' },
          },
        })
        break
      ...
      case 'schema_pass':
        ...
        set({
          edges: {
            ...s.edges,
            [e.edgeId]: { ...s.edges[e.edgeId], status: 'schema_pass' },
          },
        })
        break
      ...
      case 'schema_fail':
        ...
        set({
          edges: {
            ...s.edges,
            [e.edgeId]: { ...s.edges[e.edgeId], status: 'schema_fail' },
          },
          alerts: [alert(`Schema fail: ${e.error}`, 'error'), ...s.alerts],
        })
        break
      ...
      case 'drift_pass':
        ...
        set({
          ...
          alerts: [alert(`✓ Pass ${e.edgeId.replace(/_to_/g,' → ').replace(/_/g,' ')} (${e.score?.toFixed(3)})`, 'success'), ...s.alerts],
        })
        break
      ...
      case 'drift_fail':
        ...
        set({
          ...
          alerts: [alert(`⚠ Drift fail ${e.edgeId.replace(/_to_/g,' → ').replace(/_/g,' ')} (${e.score?.toFixed(3)} < ${e.threshold})`, 'warning'), ...s.alerts],
        })
        break
